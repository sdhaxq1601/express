+function (w) {
    function Sim(am, $obj) {//am是时间间隔的倍数，如果大于这个倍数，这个点就是节点
        ...
        var xl, dAng//斜率,变化角度
        var tempPoints = [], badAngCount = 0, ANG_DIS = Math.PI / 30
        ...
        this.apply = function (x, y, p) {
            ...
            if (p == 'move') {
                ...
                if ((Math.abs(x - px) > 2 || Math.abs(y - py) > 2)) {//过滤噪点
                    tempPoints.push({ sx: x, sy: y })
                    steps++
                    avDuration = (now - startTime) / steps
                    thisDuration = now - lastTime
                    if (thisDuration > avDuration * this.am) {
                        var tempAng = Math.asin((px - startX) / Math.sqrt((py - startY) * (py - startY) + (px - startX) * (px - startX)))
                        ...
                        for (var i = 0; i < tempPoints.length - 2; i++) {
                            ...
                            ang1 = Math.asin((tx1 - tx) / Math.sqrt((ty1 - ty) * (ty1 - ty) + (tx1 - tx) * (tx1 - tx)))
